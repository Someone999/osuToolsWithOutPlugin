...
namespace osuTools
{
    namespace Beatmaps
    {
        ...
        public partial class Beatmap
        {
            ...
            public BassMusicPlayer LoadNewMusicPlayer()
            {
                if (!File.Exists(FullAudioFileName))
                    throw new FileNotFoundException("找不到指定的文件。", FullAudioFileName);
                BassMusicPlayer musicPlayer = new BassMusicPlayer();
                musicPlayer.OnMediaFailed += (state, info, error) =>
                    throw new Exception($"在{state}文件{info.Path}时出现错误\n{error}");
                musicPlayer.Load(FullAudioFileName);
                
                return musicPlayer;
            }
            public void LoadToMusicPlayer(BassMusicPlayer player)
            {
                if (File.Exists(FullAudioFileName))
                    throw new FileNotFoundException("找不到指定的文件。",FullAudioFileName);
                player.Load(FullAudioFileName);
            }
